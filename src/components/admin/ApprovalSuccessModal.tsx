...
// Icon WhatsApp sebagai komponen SVG inline
const WhatsAppIcon = () => (
...
const ApprovalSuccessModal: React.FC<Props> = ({ data, onClose }) => {
  const { email, password, phone, message } = data;

  const handleSendWhatsApp = () => {
    // Format nomor telepon: ganti '0' di depan dengan '62' dan hapus karakter non-digit
    const formattedPhone = phone.replace(/\D/g, '').startsWith('0') 
      ? `62${phone.replace(/\D/g, '').substring(1)}` 
      : phone.replace(/\D/g, '');
    
    // Template pesan WhatsApp
    const waMessage = `Selamat! Pendaftaran Anda di BBPBAT Sukabumi telah disetujui.\n\nBerikut adalah detail akun Anda untuk login ke sistem:\nEmail: *${email}*\nPassword Sementara: *${password}*\n\nSilakan login melalui situs kami dan segera ganti password Anda untuk keamanan.\nTerima kasih.`;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-[60]">
      <div className="bg-white rounded-lg p-8 shadow-2xl max-w-md w-full m-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-gray-800">Persetujuan Berhasil</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X className="w-6 h-6"/></button>
        </div>
        <p className="mb-6 text-gray-600">{message}</p>
        
        <div className="bg-gray-100 p-4 rounded-md mb-6 space-y-3">
          <div>
            <span className="font-semibold text-gray-700">Email Peserta:</span>
            <p className="text-gray-900 break-all">{email}</p>
          </div>
          <div>
            <span className="font-semibold text-gray-700">Password Sementara:</span>
            <p className="font-mono text-lg text-blue-600 bg-blue-100 px-2 py-1 rounded inline-block">{password}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3">
          ...
          <button
            ...
            className="px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition flex items-center justify-center order-1 sm:order-2"
          >
            <WhatsAppIcon />
            Kirim via WhatsApp
          </button>
