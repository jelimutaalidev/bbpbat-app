...
// 2. Definisikan tipe data untuk state placementOptions
interface PlacementOption {
  id: number;
  nama: string;
  sisa_kuota_umum: number; // Menggunakan kuota untuk umum
}

const GeneralRegistration: React.FC<GeneralRegistrationProps> = () => {
  ...
  // 4. useEffect untuk mengambil data penempatan saat komponen dimuat
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    submissionData.append('tipe_peserta', 'UMUM'); // Hardcode tipe peserta menjadi UMUM
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Pendaftaran Masyarakat Umum/Dinas</h1>
            <p className="text-gray-600">Lengkapi formulir di bawah ini untuk mendaftar sebagai peserta umum</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label htmlFor="instansi" className="block text-sm font-medium text-gray-700 mb-2">Instansi/Perusahaan/Dinas *</label>
                <input type="text" id="instansi" name="instansi" value={formData.instansi} onChange={handleInputChange} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${errors.instansi ? 'border-red-300' : 'border-gray-300'}`} placeholder="Masukkan nama instansi/perusahaan Anda" />
              ...
              <div>
                ...
                <select id="pilihanPenempatan" name="pilihanPenempatan" value={formData.pilihanPenempatan} onChange={handleInputChange} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${errors.pilihanPenempatan ? 'border-red-300' : 'border-gray-300'}`}>
                  ...
                  {placementOptions.map((option) => (
                    <option key={option.id} value={option.id} disabled={option.sisa_kuota_umum <= 0}>
                      {option.nama} ({getQuotaText(option.sisa_kuota_umum)})
                    </option>
                  ))}
                </select>
